...
public class TestValuationOutput {
    ...
    @Test
    public void testCarValuationOutputs(){
        ...
        fileInputs.forEach(fileInput -> {
            System.out.println("=============== Running valuation checks on Registration Numbers on File:" + fileInput + " ===================="  );
            List<String> carInputFromFileList = read.storeInputAsList(fileInput, carInputFromFile);
                carInputFromFileList.stream().forEach(regNo -> {
                    if (Arrays.asList(IN_CORRECT_REG_NOS).contains(regNo)) {
                        ...
                        softly.assertThat(value.split(",")[0]).isEqualToIgnoringCase(ve.getRegistration());
                        softly.assertThat(value.split(",")[1]).isEqualToIgnoringCase(ve.getMake());
                        softly.assertThat(value.split(",")[2]).isEqualToIgnoringCase(ve.getModel());
